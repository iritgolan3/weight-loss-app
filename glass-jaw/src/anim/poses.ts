...
export function applyIntro(out: Pose, t: number, time: number): void {
  const k = clamp01(t);
  // Walks in from a crouch into the stance, then one confident glove tap.
  const rise = Ease.cubicOut(clamp01(k * 1.6));
  out.offset.y += (1 - rise) * 0.5;
  out.scale = lerp(0.92, 1, rise);
  if (k > 0.55) {
    const tap = Math.sin(clamp01((k - 0.55) / 0.45) * Math.PI);
    add(out.gloveL, -0.24 * tap, -0.05 * tap);
    add(out.gloveR, 0.24 * tap, -0.05 * tap);
    out.gloveScaleL += tap * 0.25;
    out.gloveScaleR += tap * 0.25;
  }
  out.headRot += Math.sin(time * 2.5) * 0.05;
  out.brow = 0.7;
}
